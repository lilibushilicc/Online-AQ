...
<template>
  <el-card>
    ...
    <el-table v-loading="loading" :data="feedbacks" stripe class="feedback-table">
      ...
      <el-table-column label="同题反馈" width="80" align="center">
        <template #default="{ row }">
          ...
          <span v-else class="muted feedback-empty">—</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="状态" width="85">
        <template #default="{ row }">
          <el-tag :type="getStatusTag(row.status).type" size="small" effect="plain">{{ getStatusTag(row.status).text }}</el-tag>
        </template>
      </el-table-column>
    ...
    <template v-if="detailData">
      <el-descriptions :column="1" border size="small">
        ...
        <el-descriptions-item label="状态">
          <el-tag :type="getStatusTag(detailData.feedback.status).type" size="small" effect="plain">{{ getStatusTag(detailData.feedback.status).text }}</el-tag>
      ...
      <el-divider />
      <h3 class="feedback-section-title">当前题目信息</h3>
      <el-card shadow="never" class="feedback-question-card">
        <p class="feedback-question-content"><strong>{{ detailData.question.questionContent }}</strong></p>
        <div v-if="detailData.question.questionType === 'single' || detailData.question.questionType === 'judge'" class="feedback-options">
        ...
        <div class="feedback-answer-tags">
          <el-tag effect="plain">答案：{{ detailData.question.correctAnswer }}</el-tag>
          <el-tag effect="plain">{{ detailData.question.score }} 分</el-tag>
  ...
  <el-dialog v-model="resolveDialogVisible" title="修改题目并采纳反馈" width="600px">
    <template v-if="resolveTarget">
      <p class="feedback-dialog-hint">反馈题目：<strong>{{ resolveTarget.questionContent }}</strong></p>
  ...
  <el-dialog v-model="rejectDialogVisible" title="驳回反馈" width="450px">
    <template v-if="rejectTarget">
      <p class="feedback-dialog-hint">反馈题目：<strong>{{ rejectTarget.questionContent }}</strong></p>
      <el-form label-position="top">
