...
<template>
  ...
  <el-card class="exam-list-card">
    <template #header>
      <div class="exam-list-header">
        ...
        <el-input v-model="examSearch" placeholder="搜索考试名称..." clearable class="exam-search" />
      </div>
    </template>

    <el-row :gutter="16" v-if="filteredExams.length > 0">
      <el-col v-for="exam in filteredExams" :key="exam.examId" :xs="24" :md="12" class="exam-col">
        <el-card shadow="never" class="exam-card">
          <template #header>
            <div class="exam-card__header">
              <strong class="exam-card__name">{{ exam.examName }}</strong>
              <el-tag :type="exam.status === 'published' ? 'success' : exam.status === 'closed' ? 'info' : 'warning'" size="small" effect="plain">
                {{ exam.status === 'published' ? '已发布' : exam.status === 'closed' ? '已关闭' : '草稿' }}
              </el-tag>
            </div>
          </template>
          ...
          <div class="exam-meta">
            ...
            <div class="exam-meta__item">
              ...
              <span class="exam-meta__value">{{ exam.allowRetake ? '允许' : '禁止' }}</span>
            </div>
          </div>

          <div class="exam-meta">
            <div class="exam-meta__item">
              <span class="exam-meta__label">乱序</span>
              <span class="exam-meta__value">{{ [exam.shuffleQuestions && '题目', exam.shuffleAnswers && '选项'].filter(Boolean).join('+') || '无' }}</span>
            </div>
            <div class="exam-meta__item">
  ...
  <el-dialog v-model="publishDialogVisible" title="发布考试" width="500px">
    <template v-if="publishExamTarget">
      <p class="dialog-label">考试：<strong>{{ publishExamTarget.examName }}</strong></p>
      <el-radio-group v-model="publishMode" class="publish-mode-group">
        <el-radio value="all">全体学生</el-radio>
        <el-radio value="select">仅指定学生</el-radio>
      </el-radio-group>

      <template v-if="publishMode === 'select'">
        <el-checkbox-group v-model="publishStudentIds" class="student-checkbox-group">
          <el-checkbox v-for="s in studentOptions" :key="s.userId" :value="s.userId">
            {{ s.realName }}（{{ s.username }}）
          </el-checkbox>
        </el-checkbox-group>
        <div v-if="publishStudentIds.length === 0" class="validation-hint">
          <span class="muted">请至少选择一名学生</span>
        </div>
      </template>

      <el-divider />
      <p class="section-label">乱序配置（发布后可关闭再发布不同版本）</p>
  ...
  <el-dialog v-model="settingsDialogVisible" title="考试设置" width="460px">
    <template v-if="settingsExam">
      <p class="dialog-label">考试：<strong>{{ settingsExam.examName }}</strong></p>
  ...
  <el-drawer v-model="previewVisible" title="试卷预览" size="42%">
    <template v-if="previewExam">
      ...
      <div class="question-list">
        <el-card v-for="(question, index) in previewQuestions" :key="question.questionId" shadow="never" class="preview-question-card">
  ...
  <el-drawer v-model="historyVisible" title="考试历史记录" size="42%">
    <template v-if="previewExam">
      ...
      <el-divider />
      <div class="question-list">
        <el-card v-for="history in currentHistory" :key="history.historyId" shadow="never" class="history-card">
