# 文档索引

## 项目说明

| 文档 | 说明 |
|---|---|
| `README.md`（根目录） | 项目总览、技术栈、启动方式、测试账号 |
| `AGENTS.md`（根目录） | 项目协作规则与文档约定 |
| `docs/项目整体说明.md` | 项目定位、三端结构、模块组成与当前状态总览 |
| `docs/api-reference.md` | 全部 83 个接口参考文档 |
| `docs/考试模块.md` | 考试、草稿、重考、稳定乱序映射说明 |
| `docs/question-feedback-feature-design.md` | 题目纠错反馈功能完整设计文档 |

## 代码改动总结（docs/change-summary/）

每次代码改动后生成的总结文档，按日期倒序排列：

| 文件名 | 日期 | 内容 |
|---|---|---|
| `2026-05-23-1628-项目整体说明与README重写-改动总结.md` | 05-23 | 新增项目整体说明，重写 README 总览文档 |
| `2026-05-23-1612-统一鉴权与结果链路类型收敛-改动总结.md` | 05-23 | `@AdminOnly` 统一鉴权、结果链路 VO 化、前端构建恢复全绿 |
| `2026-05-23-1608-答案返回策略收口-改动总结.md` | 05-23 | 答题明细正确答案改为显式控制返回 |
| `2026-05-23-1558-考试与后台管理安全校验修复-改动总结.md` | 05-23 | 考试详情答案脱敏、后台接口补齐管理员校验 |
| `2026-05-23-1545-考试重考与稳定乱序映射修复-改动总结.md` | 05-23 | 重考正式支持、attemptId 会话与稳定乱序映射 |
| `2026-05-21-1000-exam-create-fix-and-login-update-summary.md` | 05-21 | 修复试卷创建 NPE、登录页移除教师入口 |
| `2026-05-20-2236-announcement-feature-summary.md` | 05-20 | 系统公告功能 |
| `2026-05-20-2201-export-word-excel-fix-summary.md` | 05-20 | 修复 Word/Excel 导出 |
| `2026-05-20-1510-user-profile-summary.md` | 05-20 | 个人中心与密码修改 |
| `2026-05-20-1700-email-registration-summary.md` | 05-20 | 邮箱学生注册 |
| `2026-05-20-1435-code-optimization-summary.md` | 05-20 | 代码优化（后端通用方法、前端组合函数） |
| `2026-05-20-1430-smtp-send-limits-summary.md` | 05-20 | SMTP 发送限制 |
| `2026-05-20-1400-smtp-accounts-summary.md` | 05-20 | 多 SMTP 账号管理 |
| `2026-05-20-1210-考试管理去重改动总结.md` | 05-20 | 考试管理与试卷管理职责分离 |
| `2026-05-19-2000-code-cleanup-summary.md` | 05-19 | Element Plus 全量导入、代码清理 |
| `2026-05-19-2005-admin-upload-ui-refactor-summary.md` | 05-19 | 上传试题页 UI 重构 |
| `2026-05-19-1945-ai-upload-parsing-fix-summary.md` | 05-19 | AI 上传解析优化 |
| `2026-05-19-1930-batch-improvements-summary.md` | 05-19 | 草稿保存、时间校准、分页、profile 拆分 |
| `2026-05-19-1745-online-question-bank-fix-summary.md` | 05-19 | 修复分类过滤 bug |
| `2026-05-19-1700-shuffle-questions-answers-summary.md` | 05-19 | 题目/选项乱序防作弊 |
| `2026-05-19-1630-student-export-features-summary.md` | 05-19 | 错题本/成绩导出 Excel |
| `2026-05-19-1630-security-quality-performance-ui-improvements-summary.md` | 05-19 | 安全/性能/UI 综合优化 |
| `2026-05-19-1615-ai-question-parsing-summary.md` | 05-19 | AI 智能解析试题 |
| `2026-05-19-1600-custom-score-and-wrong-book-summary.md` | 05-19 | 自定义分值与错题本 |
| `2026-05-19-1044-wrong-notebook-multi-and-nav-optimization.md` | 05-19 | 错题本多本支持、导航优化 |
| `2026-05-18-2259-question-feedback-feature-summary.md` | 05-18 | 题目纠错反馈 |
| `2026-05-18-2253-frontend-ui-redesign-summary.md` | 05-18 | 前端 UI 全面重构 |
| `2026-05-18-2104-android-student-app-summary.md` | 05-18 | Android 学生端 App |
| `2026-05-18-1730-ui-optimization-summary.md` | 05-18 | UI 优化（配色/字体/Bento/Liquid Glass） |
| `2026-05-18-1725-question-category-feature-summary.md` | 05-18 | 题目分类功能 |
| `2026-05-17-2341-登录修复改动总结.md` | 05-17 | 登录修复（认证异常、Axios 拦截器） |
| `2026-05-17-2320-login-routing-fix-summary.md` | 05-17 | 修复前端请求路径配置 |
| `2026-05-17-2309-login-failure-fix-summary.md` | 05-17 | 自动补齐默认测试账号 |
| `2026-05-17-2307-login-error-fix-summary.md` | 05-17 | 统一错误处理 |
| `2026-05-17-2155-first-priority-features-and-history-summary.md` | 05-17 | 考试时间窗口、重考策略、考试历史 |
| `2026-05-17-2139-code-optimization-summary.md` | 05-17 | 首次代码优化 |
| `2026-05-17-0013-project-document-rule-summary.md` | 05-17 | 项目文档规则初始建立 |

## 实施计划（docs/superpowers/plans/）

| 文件名 | 说明 |
|---|---|
| `2026-05-17-timer-history-lazyload.md` | 答题倒计时、历史记录补全、路由懒加载与构建分包 |

## 文档导航

- 查看项目概览与启动方式 → `README.md`
- 查看协作规则 → `AGENTS.md`
- 查看项目整体说明 → `docs/项目整体说明.md`
- 查看考试模块说明 → `docs/考试模块.md`
- 查看功能设计 → `docs/question-feedback-feature-design.md`
- 查看代码改动历程 → `docs/change-summary/`
- 查看未来计划 → `docs/superpowers/plans/`
- 查看日志文件 → `log/`
- 查看后续规划 → `WillPlan/`
